...
If you wish to setup the task yourself or dive deeper into the block type tasks, this part of the tutorial is for you.


There is, first, a few steps to setup a task. Those are :

1. Create the task. (The tutorial for this can be found here https://docs.inginious.org/en/latest/teacher_doc/task_tuto.html ) Set up a title, a context, your name, the options you want,... When creating a subproblem, select "blockly" as "type of task", give it an ID and don't forget to save by pressing "Save changes"
2. If you want, in the "Workspace option" button, set up the maximum number of blocks that the student can use to perform the task by entering it in the "Max number of blocks" field (by default, it is "Infinity")

.. image:: ../images/img_en/block/number_of_blocks.png
   :align: center
   :width: 75 %

3. Don't forget to change the "environment" accordingly. For example in a blockly task, you would choose "Standard container (Docker)" for the "Grading environment type". You also need to change the "Grading environment" based on the language you'll use. Put "Default" if unsure. 
...
Customise your task : how to create the sum function (using the graphical interface)
------------------------------------------------------------------------------------
...
Customise your task :  create the sum function by hand (using xml)
------------------------------------------------------------------
...
Customise your task : an "only workspace" task
----------------------------------------------
...
Customise your task : how to create a custom block (if/else)
------------------------------------------------------------

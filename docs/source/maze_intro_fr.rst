...
* 4. Afin de tester sa solution, l'étudiant doit appuyer sur **Submit**. Le fait d'appuyer sur **Exécuter le code** ne fait que montrer sur la carte l'effet des blocs. Le bouton **Submit** déclenche les scripts de notation, montrant le feedback que vous avez décidé de définir en fonction de la réponse de l'étudiant (le système de notation est abordé plus en détail plus loin dans ce tutoriel).

...
Ce tutoriel a pour but de vous aider à naviguer dans les informations nécessaires à la créations de tâches blockly orientées blocs.
...
* 1. Le bouton **"Exécuter le code"** permet à l'étudiant d'exécuter les blocs présents dans **l'espace de travail**. Pour exécuter le code plusieurs fois, l'élève doit appuyer à nouveau sur le bouton lorsqu'il est indiqué **"Redémarrer "** puis **"Exécuter le code "** à nouveau.
...
Comme expliqué précédemment, le fait d'appuyer sur **"Exécuter le code "** ne fait qu'exécuter le code afin que l'étudiant puisse voir ce que les blocs présents dans l'espace de travail font en conséquence.
